-- ====================================================================
-- Migration: 20260524_01_atomic_consume_stock_fifo.sql
-- ====================================================================
-- Resolve race condition and partial consumption by making the fallback
-- for items without lots fully atomic inside the RPC.
...
begin
  ...
  -- Fallback atômico: se ainda sobrar quantidade (ex: itens legado sem lotes)
  if v_remaining > 0 then
    ...
    values (
      p_stock_item_id, p_stock_item_id, null, p_tipo, v_remaining, v_unidade,
      null, trim(coalesce(p_observacao, '') || ' (consumo sem lote)'), p_origem, v_user_id
    );

    update public.stock_items
    set quantidade = greatest(0, quantidade - v_remaining)
    where id = p_stock_item_id;

    v_consumed_total := v_consumed_total + v_remaining;
  end if;

  -- Os triggers trg_sync_stock_item_* recalculam quantidade a partir dos lotes modificados.
  -- Para o fallback, o update manual garante o recálculo do que não tem lote.
